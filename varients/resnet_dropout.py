...
writer=SummaryWriter('resnet_logs_dropout')
...
class residual_connection(nn.Module):
    def __init__(self,in_channel,hidden_channel,out_channel,kernel_size=3,stride=1,padding=1,dropout_rate=0.2):
        super().__init__()
...
class resnetblock(nn.Module):
    def __init__(self,in_channel,hidden_channel,out_channel,num_residual_connection=2,kernel_size=3,stride=1,padding=1,dropout_rate=0.2):
        super().__init__()
        self.residual_connections=nn.ModuleList([residual_connection(in_channel,hidden_channel,out_channel,kernel_size,stride,padding,dropout_rate) for _ in range(num_residual_connection)])
    def forward(self,x):
...
class resnet(nn.Module):
    def __init__(self,dropout_rate=0.2):
        ...
        self.resnetblock1=resnetblock(32,16,32,2,5,1,padding=2,dropout_rate=dropout_rate)
        self.pool1 = nn.MaxPool2d(2)
        self.silu1 = nn.SiLU()

        # self.dropout2 = nn.Dropout2d(dropout_rate)
        # self.conv2 = nn.Conv2d(32, 32, 5, padding=2)
        self.resnetblock2=resnetblock(32,16,32,2,5,1,padding=2,dropout_rate=dropout_rate)
        self.pool2 = nn.MaxPool2d(2)
        self.silu2 = nn.SiLU()

        self.dropout3 = nn.Dropout2d(dropout_rate)
        self.conv3 = nn.Conv2d(32, 64, 3, padding=1)
        self.resnetblock3=resnetblock(64,16,64,2,5,1,padding=2,dropout_rate=dropout_rate)
